    ...
    function($) {
        ...
        $.fn.fitVids = function(options) {
            ...
            return options && $.extend(settings, options), this.each(function() {
                var selectors = ['iframe[src*="player.vimeo.com"]:not(.depicter-video-player)', 'iframe[src*="youtube.com"]:not(.depicter-video-player)', 'iframe[src*="youtube-nocookie.com"]:not(.depicter-video-player)', 'iframe[src*="kickstarter.com"][src*="video.html"]', "object", "embed"];
...
function(window) {
        ...
        $.extend(Plugin.prototype, {
            ...
            update: function(event) {
                var rect = this.element.getBoundingClientRect(),
                    ...
                    remainingPageScroll = document.documentElement.scrollHeight - (window.scrollY + window.innerHeight),
                    passedPageScroll = window.scrollY,
                    areaTop = Math.min(this._parseOffset(this._offsetTop), passedPageScroll),
                    areaBottom = window.innerHeight - Math.min(this._parseOffset(this._offsetBottom), remainingPageScroll);
    ...
    function(global, factory) {
        ...
        function updateGeometry(i) {
            ...
            i.containerWidth = Math.ceil(rect.width), i.containerHeight = Math.ceil(rect.height), i.contentWidth = element.scrollWidth, i.contentHeight = element.scrollHeight, element.contains(i.scrollbarXRail) || (queryChildren(element, cls.element.rail("x")).forEach(function(el) {
                    return remove(el)
    ...
    function($, window) {
        ...
        p.appendItem = function(item) {
            ...
            if (op.loop) {
                if (before - space < 2 * this[this._dimension]) return op.loop = !1, this.scroller.options.endless = !1, void this._updateView()
            } else this.scroller._max_value = before - space - this[this._dimension], middlePosition = 0, this._spaceBuffer = 0, op.center && (this.scroller._max_value = before - this[this._dimension] / 2 + this.items[0].size);
            for (i = 0; i !== l; i++)(item = this.items[i]).position = this._spaceBuffer + item.position - middlePosition, item.$element[0].style[this._offset] = item.position + "px", item.$element[0].style[this._dimension] = item.size + "px"
